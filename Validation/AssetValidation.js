...
const baseAssetSchema = Joi.object({
  ...
  asset_code: Joi.string().max(50).allow('', null),
  ...
  asset_status: Joi.string().max(50).allow('', null),
  asset_condition: Joi.string().max(50).allow('', null),

  quantity: Joi.number().integer().allow(null),
  price: Joi.number().precision(2).allow(null),

  asset_photo: Joi.string().max(255).allow('', null),
  asset_image_url: Joi.string().uri().allow('', null),

  description: Joi.string().allow('', null),
  allocated_to: Joi.string().max(100).allow('', null),
  purchased_date: Joi.date().iso().allow(null),
  purchased_by: Joi.string().max(50).allow('', null),
...
// Create schema
const createAssetSchema = baseAssetSchema.keys({
  created_by: Joi.string().max(100).required()
});

// Update schema
const updateAssetSchema = baseAssetSchema.keys({
